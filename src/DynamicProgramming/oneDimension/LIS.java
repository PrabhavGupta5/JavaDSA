...
public class LIS {
    ...
    public int findLongestChain(int[][] pairs) {
        // Main trick in this problem is to sort the pairs based on the first element of the pair, because we need to compare the first element of the current pair with the second element of the previous pair, so we need to sort the pairs based on the first element to ensure that we are comparing the correct elements, and we are using a lambda expression to sort the pairs in ascending order based on the first element of the pair, which is equivalent to writing a comparator class that implements the Comparator interface and overrides the compare method to compare the first elements of the pairs, but using a lambda expression is more concise and easier to read.
        Arrays.sort(pairs, (a,b) -> a[0] -b[0]); // we are sorting the pairs based on the first element of the pair, because we need to compare the first element of the current pair with the second element of the previous pair, so we need to sort the pairs based on the first element to ensure that we are comparing the correct elements, and we are using a lambda expression to sort the pairs in ascending order based on the first element of the pair, which is equivalent to writing a comparator class that implements the Comparator interface and overrides the compare method to compare the first elements of the pairs, but using a lambda expression is more concise and easier to read.
